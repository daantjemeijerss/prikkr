...
export default function ResultsPage() {
  ...
  async function fetchMeta() {
    ...
    setSlotDuration(meta.slotDuration); 
  }
...
const durationInMinutes = (value: string | number): number => {
  ...
  return map[value] ?? 60; // default fallback
};

const generateSlots = () => {
  if (slotDuration === 'daily' || slotDuration === '1440') return ['All Day'];

  const step = parseInt(String(slotDuration)); // handle number or string input
  const slots: string[] = [];
  const startHour = 9;
...
{range && sortedDates.length > 0 && Object.keys(allSlots).length > 0 && (
  slotDuration?.toString() === '1440' ? (
    <>
      {Object.entries(groupDatesByWeek(sortedDates)).map(([weekLabel, datesInWeek]) => (
        <div
          key={weekLabel}
          className="bg-white rounded-xl shadow-xl px-3 py-2 mb-6 w-full max-w-screen-xl mx-auto"
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2">
            {datesInWeek.map((date) => {
              ...
              return (
                <div
                  key={date}
                  className={`rounded-xl p-4 text-sm sm:text-base font-medium text-center shadow-[0_8px_20px_rgba(0,0,0,0.25)] ${bgColor}`}
                >
  ...
  <>
    {Object.entries(groupDatesByWeek(sortedDates)).map(([weekLabel, datesInWeek]) => (
      <div
        key={weekLabel}
        className="bg-white rounded-2xl shadow-md px-3 py-2 mb-6 w-full max-w-screen-xl mx-auto"
      >
        ...
        <div className="space-y-2">
          {datesInWeek.map((date) => {
            ...
            return (
              <div
                key={date}
                className="bg-white rounded-xl shadow-xl px-4 py-1 w-full"
              >
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 w-full">
                  ...
                  <div
                    className={`grid w-full
                      ...
                      ${mdCols} ${lgCols}
                      gap-x-1 gap-y-0 sm:gap-x-1 sm:gap-y-1
                    `}
                  >
                    {fullSlots.map((time) => {
  ...
  return (
    <div
      key={time}
      className={`min-w-0 px-2 py-4 text-[10px] sm:text-sm font-semibold 
